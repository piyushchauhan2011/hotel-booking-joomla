...
- Booking workflow: Components → Hotel Booking → Options has **Enable Workflows** on. Components → Workflows lists **Booking lifecycle**. Submit a guest booking on the site, then in admin Bookings the stage is **Received**. Open the booking: **Notify Hotel** is on the toolbar (same durable transition as list Actions → **Notify hotel**). Confirm Mailpit, then **Hotel confirms** → **Mark paid** until **Confirmed**. Second pass: clear a destination's partner email, run Notify Hotel — the stage stays Received; set the email and retry. Open the workflow graph.
